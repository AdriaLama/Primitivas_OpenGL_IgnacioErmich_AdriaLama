...
void Scene::Setup()
{
  /*  cube.position = glm::vec3(-0.55f, 0.f, 0.f);
    ...
    trollColors[0] = glm::vec4(0.2, 1.0, 0.3, 1);
    trollColors[1] = glm::vec4(0.2, 0.7, 0.7, 1);
    trollColors[2] = glm::vec4(0.2, 0.4, 1.0, 1);
...
void Scene::Render()
{
    ...
    glm::mat4 trollScale = GenerateScaleMatrix(glm::vec3(0.3f, 0.3f, 0.3f));
    ...
    RM->SetColor(glm::vec4(1.f, 1.f, 0.f, 1.f));
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        glm::mat4 trollMatrix = GenerateTranslationMatrix(trollPositions[i]) * GenerateRotationMatrix(glm::vec3(0.f, 1.f, 0.f), trollRotations[i]) * trollScale;
        RM->DrawModel(models[0], trollMatrix, projection, view);
